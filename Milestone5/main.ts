...
// print button
document.getElementById("download-btn")?.addEventListener("click", printPage);

// Function to share the current page link
function sharePage() {
    const pageUrl = window.location.href; // Get the current page URL
    navigator.clipboard.writeText(pageUrl) // Copy the URL to the clipboard
        .then(() => {
            // Display a message to the user after successful copying
            const shareMessage = document.getElementById('share-btn');
            if (shareMessage) {
                shareMessage.textContent = "Page link copied to clipboard!";
            }
        })
        .catch((err) => {
